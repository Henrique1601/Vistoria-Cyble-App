      ...
      >
        ...
        <div className="bg-base-raised border border-base-border rounded-2xl p-6 shadow-diffusion">
          <div className="flex items-center gap-2 mb-4">
            <LockKey size={16} weight="duotone" className="text-content-tertiary" aria-hidden="true" />
            <label htmlFor="pin-input" className="text-xs font-semibold uppercase tracking-widest text-content-tertiary">PIN</label>
          </div>
          <input
            id="pin-input"
            type="password"
            ...
            autoComplete="off"
            placeholder="****"
            aria-label="Codigo PIN de acesso"
            aria-describedby={erro ? 'pin-error' : undefined}
            aria-invalid={!!erro}
            value={pin}
            onChange={(e) => setPin(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && pin && !checking && confirmar()}
            className="w-full bg-base-overlay border border-base-border rounded-xl px-4 py-3.5 text-center text-lg font-mono tracking-[0.5em] text-content placeholder:text-content-tertiary/50 focus:outline-none focus:border-accent/50 focus-visible:ring-2 focus-visible:ring-accent focus-visible:outline-none focus:shadow-glow-accent transition-all"
          />
          ...
          <button
            ...
            aria-label={checking ? 'Verificando PIN' : 'Entrar'}
            className="tactile-press w-full mt-5 bg-accent text-base font-semibold text-sm rounded-xl px-6 py-3.5 hover:bg-accent-hover focus-visible:ring-2 focus-visible:ring-accent focus-visible:outline-none transition-colors disabled:opacity-30 disabled:pointer-events-none"
          >
            {checking ? (
              <span className="flex items-center justify-center gap-2">
                <span className="w-4 h-4 border-2 border-base/30 border-t-base rounded-full animate-spin" aria-hidden="true" />
                Verificando\u2026
              </span>
            ) : (
              'Entrar'
